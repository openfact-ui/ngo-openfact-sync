...
[![Build Status](https://ci.centos.org/buildStatus/icon?job=devtools-ngo-openfact-sync-npm-publish-build-master)](https://ci.centos.org/view/Devtools/job/devtools-ngo-openfact-sync-npm-publish-build-master/)
[![npm version](https://badge.fury.io/js/ngo-openfact-sync.svg)](https://badge.fury.io/js/ngo-openfact-sync)
...
You can see how it is used in the front-end [here](https://github.com/openfactui/openfact-ui).
...
To build ngo-openfact-sync as a npm library, use:
...
To build ngo-openfact-sync as an npm library and embed it into a webapp such as
openfact-ui, you should:

1. Run `npm run watch:library` in this directory. This will build ngo-openfact-sync as
a library and then set up a watch task to rebuild any ts, html and scss files you
change.
2. In the webapp into which you are embedding, run `npm link <path to ngo-openfact-sync>/dist-watch`.
This will create a symlink from `node_modules/ngo-openfact-sync` to the `dist-watch` directory
and install that symlinked node module into your webapp.
3. Run your webapp in development mode, making sure you have a watch on `node_modules/ngo-openfact-sync`
...
In ngo-openfact-sync we use the [semantic-release plugin](https://github.com/semantic-release/semantic-release). That means 
